...
> | # | 配置项 | 说明 | 示例 |
> |---|--------|------|------|
> | 1 | **API 服务地址** | 实际接口的后端服务 URL（内网/服务器地址） | `http://192.168.200.96:15015/项目名` |
...
6. **两级测试设计** — 单接口用例（正向25%/逆向40%/边界20%/安全15%）+ 业务流程用例（多接口串联）
...
7. **自动报告** — 测试完成后自动生成 Excel 测试用例（2 Sheet：单接口/业务流程，各 10 列）和 Word 测试报告（6 章节），无需手动调用
...
- 分析接口定义，生成**单接口用例**（正向25%/逆向40%/边界20%/安全15%）
