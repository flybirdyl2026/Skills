...
def _load_fallback_config():
    ...
    # 向上三级从 scripts/到达 skills/ 父目录
    skill_parent = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
...
def write_test_cases_to_excel(module_test_cases, output_file='test_cases.xlsx', problems=None):
    ...
    for module_name, result in module_test_cases:
        ...
        # ---- 提取需求编号（括号内的任意编码）----
        req_code_match = re.search(r'[（(]([^)）]+)[）)]', module_name)
        REQ_CODE = req_code_match.group(1) if req_code_match else ''
